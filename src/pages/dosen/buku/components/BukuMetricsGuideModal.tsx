        ...
        >
          ...
          <div className="px-6 sm:px-8 py-5 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between bg-slate-50/50 dark:bg-zinc-900/50 shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-100 dark:border-amber-900/40 flex items-center justify-center text-amber-600 dark:text-amber-400 shadow-inner">
                <BarChart2 className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight">
                  Panduan Metriks Penilaian KPI Buku Akademik
                </h3>
                <p className="text-[11px] font-bold text-slate-400 dark:text-zinc-400 uppercase tracking-wider">
                  Metriks Buku Dosen (Dokumen Internal)
                </p>
              </div>
            </div>
          ...
          <div className="p-6 sm:p-8 overflow-y-auto space-y-6 custom-scrollbar">
            {/* Top Info Banner */}
            <div className="p-5 bg-gradient-to-r from-amber-500/10 via-indigo-500/10 to-emerald-500/10 border border-slate-200 dark:border-zinc-800 rounded-3xl relative overflow-hidden">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-medium text-slate-600 dark:text-zinc-400 mt-0.5 leading-relaxed">
                      Poin dihitung secara otomatis berdasarkan jenis buku yang terdaftar dan telah disetujui oleh administrator LPPM.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-zinc-900 p-6 sm:p-8 rounded-[2rem] border border-slate-100 dark:border-zinc-800 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 bg-amber-500/10 rounded-2xl flex items-center justify-center border border-amber-500/20 shadow-inner">
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/50 text-xs font-bold text-slate-700 dark:text-zinc-300">
                    {BUKU_CATEGORIES.map((cat) => (
                      <tr key={cat.value} className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                        <td className="py-3">
                          <div>
                            <span className="block font-bold">{cat.label}</span>
                            {cat.desc && <span className="block text-[10px] font-normal text-slate-400 dark:text-zinc-500">{cat.desc}</span>}
                          </div>
                        </td>
                        <td className="py-3 text-right text-amber-600 dark:text-amber-400 font-black text-sm">{cat.points}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
