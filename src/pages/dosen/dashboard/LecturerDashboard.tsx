  ...
  return (
    <div className="w-full space-y-6 pb-20">
      ...
      <motion.div
        ...
        className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-8 shadow-xs"
      >
        {/* Top Profile Info Section */}
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          {/* Avatar */}
          <div className="h-20 w-20 shrink-0 overflow-hidden rounded-2xl border border-slate-200/80 dark:border-slate-700/80 bg-slate-100 dark:bg-slate-800 sm:h-22 sm:w-22">
            {activeUser?.avatar ? (
              <img
              ...
              <div className="flex h-full w-full items-center justify-center bg-slate-900 text-2xl font-bold text-white dark:bg-zinc-100 dark:text-slate-900">
        ...
        <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-3">
          {statsLocal.map((stat, i) => (
            <div
              key={i}
              className="flex min-h-[80px] items-center gap-3.5 rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-850/50 p-4 transition-all duration-200 hover:border-slate-300 dark:hover:border-slate-700"
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-200/80 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300">
              ...
              <div className="min-w-0 flex-1">
                <phantom-ui loading={loading && !profileData} animation="shimmer" className="block space-y-0.5">
                  ...
                  <span className="block text-xl font-bold font-mono leading-tight tracking-tight text-slate-900 dark:text-white tabular-nums">
                    {stat.val ?? '0'}
                  </span>
                </phantom-ui>
              </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          {/* Google Scholar Card */}
          <div className="flex flex-col justify-between rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-850/50 p-5">
            <div>
                ...
                {scholarData ? (
                  <div className="grid grid-cols-3 gap-3">
                    <div className="rounded-xl border border-slate-200/80 bg-white p-3 text-center dark:border-slate-800 dark:bg-slate-900">
                      ...
                      <span className="mt-1 block text-xl font-bold font-mono tracking-tight text-slate-900 dark:text-white tabular-nums">
                  ...
                  <div className="flex flex-col items-center justify-center py-6 text-center rounded-xl border border-dashed border-slate-200/80 bg-white dark:border-slate-800 dark:bg-slate-900">
                    <BookOpen className="h-6 w-6 text-slate-400 mb-1.5" />
                    <p className="text-xs text-slate-500 dark:text-slate-400">Belum ada data terhubung</p>
                  </div>
                )}
              </phantom-ui>
            </div>
            ...
            {scholarData?.last_synced && (
              <div className="text-[11px] font-mono text-slate-400 dark:text-slate-500 text-right mt-3">
            ...
            <div>
              ...
              <phantom-ui loading={loading && !profileData} animation="shimmer" className="block">
                {scopusData ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-6 text-center rounded-xl border border-dashed border-slate-200/80 bg-white dark:border-slate-800 dark:bg-slate-900">
                    <Globe className="h-6 w-6 text-slate-400 mb-1.5" />
                    <p className="text-xs text-slate-500 dark:text-slate-400">Belum ada data terhubung</p>
                  </div>
                )}
              </phantom-ui>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-slate-700 dark:text-slate-300">
                  Internal:{' '}
                  <strong className="font-bold text-slate-900 dark:text-white tabular-nums font-mono">
                    {internalPoints.toLocaleString()} pts
                  </strong>{' '}
                  <span className="text-slate-500 dark:text-slate-400">({internalPct.toFixed(1)}%)</span>
                </span>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-slate-700 dark:text-slate-300">
                  ...
                  <span className="text-slate-500 dark:text-slate-400">({apiPct.toFixed(1)}%)</span>
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-2">
        <div className="grid grid-cols-2 sm:flex w-full sm:w-auto p-1 bg-slate-100 dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700 gap-1">
          {[
