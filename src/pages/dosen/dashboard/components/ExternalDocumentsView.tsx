    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {showBreakdown && (
          ...
          >
            <p className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2">Rincian Perhitungan Poin (SINTA GS):</p>
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">Dokumen GS</span>
              <span className="text-[10px] font-black text-blue-600">+0.50 pts</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">Dokumen Tersitasi</span>
              <span className="text-[10px] font-black text-blue-600">
                +{((doc.citations || 0) > 0 ? 0.50 : 0.00).toFixed(2)} pts
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">
                Sitasi (×{Math.min(doc.citations || 0, 500)} × 0.25)
                {(doc.citations || 0) > 500 && ' (Cut-off 500)'}
              </span>
              <span className="text-[10px] font-black text-blue-600">
                +{(Math.min(doc.citations || 0, 500) * 0.25).toFixed(2)} pts
              </span>
            </div>
            <div className="pt-1.5 mt-1 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
              ...
              <span className="text-[10px] font-black text-blue-700 dark:text-blue-400">= {docPoints.toFixed(2)} pts</span>
            </div>
          </motion.div>
        )}
      </div>
      <a
        href={doc.link || '#'}
        target="_blank"
        className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-400 hover:bg-blue-600 hover:text-white transition-all flex-shrink-0"
      >
  ...
  return (
    ...
    >
      <div className="w-14 h-14 rounded-2xl bg-emerald-50 dark:bg-slate-800 flex flex-col items-center justify-center border border-emerald-100 dark:border-slate-700 flex-shrink-0">
        <span className="text-lg font-black text-slate-900 dark:text-white leading-none">{bd.citations}</span>
        <span className="text-[7px] font-black text-slate-400 uppercase tracking-tighter mt-1">Sitasi</span>
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-3 mb-2">
        ...
        {showBreakdown && (
          ...
          >
            ...
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">{bd.detailStr}</span>
              <span className="text-[10px] font-black text-emerald-600">+{bd.basePoints} pts</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">Sitasi ({bd.citations} sitasi / {bd.totalAuthors} penulis)</span>
              <span className="text-[10px] font-black text-emerald-600">+{bd.citationPoints.toFixed(2)} pts</span>
            </div>
            {bd.citations > 0 && (
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">Bonus Dokumen Tersitasi</span>
                <span className="text-[10px] font-black text-emerald-600">+5.00 pts</span>
              </div>
            )}
            <div className="pt-1.5 mt-1 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
              <span className="text-[10px] font-black text-slate-700 dark:text-slate-200 uppercase">Total</span>
              <span className="text-[10px] font-black text-emerald-700 dark:text-emerald-400">= {bd.totalPoints.toFixed(2)} pts</span>
            </div>
          </motion.div>
        )}
      </div>
                  ...
                  <>
                    ...
                    {/* Document List */}
                     <div className="space-y-5">
                       {/* === Aggregate Stats Bar === */}
                       {(() => {
                         const totalDocs = scopusList.length;
                         const totalPts  = scopusList.reduce((a: number, d: any) => a + calculateScopusBreakdown(d).totalPoints, 0);
                         const totalCit  = scopusList.reduce((a: number, d: any) => a + (Number(d.citations) || 0), 0);
                         const q1Count   = scopusList.filter((d: any) => d.quartile === 'Q1').length;
                         const q2Count   = scopusList.filter((d: any) => d.quartile === 'Q2').length;
                         const crossCnt  = scopusList.filter((d: any) => crossIndexedDocs.some((c: any) => normalizeTitle(c.title) === normalizeTitle(d.title))).length;
                         const statItems = [
                           { label: 'Total Dokumen', value: String(totalDocs),          sub: 'publikasi Scopus',    accent: 'orange'  },
                           { label: 'Total Poin',    value: `${totalPts.toFixed(1)}`,   sub: 'pts SINTA',          accent: 'amber'   },
                           { label: 'Total Sitasi',  value: String(totalCit),           sub: 'dampak ilmiah',      accent: 'blue'    },
                           { label: 'Q1 + Q2',       value: String(q1Count + q2Count),  sub: `${q1Count} Q1 · ${q2Count} Q2`, accent: 'emerald' },
                           { label: 'Cross-Indexed', value: String(crossCnt),           sub: 'juga di Scholar',    accent: 'violet'  },
                         ];
                         return (
                           <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                             {statItems.map(s => (
                               <div key={s.label} className={`p-3.5 bg-${s.accent}-50 dark:bg-${s.accent}-950/20 rounded-2xl border border-${s.accent}-100 dark:border-${s.accent}-900/30`}>
                                 <p className={`text-[8px] font-black text-${s.accent}-500 uppercase tracking-widest mb-1 truncate`}>{s.label}</p>
                                 <p className={`text-xl font-black text-${s.accent}-700 dark:text-${s.accent}-300 leading-none`}>{s.value}</p>
                                 <p className={`text-[8px] font-bold text-${s.accent}-400 mt-1.5 truncate`}>{s.sub}</p>
                               </div>
                             ))}
                           </div>
                         );
                       ...
                       <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 bg-gradient-to-r from-orange-50 to-amber-50/60 dark:from-orange-950/20 dark:to-amber-950/10 border border-orange-100 dark:border-orange-900/30 rounded-2xl">
                         ...
                         <div className="flex flex-wrap gap-2 flex-shrink-0">
                           {[
                             { label: 'Q1 First Author', value: '24 pts' },
                             { label: 'Sitasi', value: 'cit ÷ n pts' },
                             { label: 'Bonus Tersitasi', value: '+5 pts' },
                           ].map(pill => (
                             <div key={pill.label} className="px-3 py-1.5 bg-white dark:bg-slate-900 rounded-xl border border-orange-100 dark:border-orange-900/30 text-center min-w-[80px]">
                               <p className="text-[8px] font-black text-orange-500 uppercase tracking-wide">{pill.label}</p>
                               <p className="text-[11px] font-black text-slate-800 dark:text-slate-100">{pill.value}</p>
                             </div>
                           ))}
                         </div>
                       </div>
                      ...
                      <div className="flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 rounded-2xl">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-blue-600/70 dark:text-blue-400/70 mt-1">
                            +0.5 per dokumen &nbsp;·&nbsp; +0.25 per sitasi (max 500) &nbsp;·&nbsp; +0.5 bonus tersitasi
                          </p>
                        </div>
                      </div>
