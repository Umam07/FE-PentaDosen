...
import { Trash2, Beaker, Loader2 } from 'lucide-react';
...
export default function ResearchDeleteModal({
  ...
  const programLabel = deleteDoc?.program 
    ? (deleteDoc.program === 'hibah internal' ? 'Internal' : deleteDoc.program === 'hibah dikti' ? 'Dikti' : deleteDoc.program === 'hibah luar negeri' ? 'Luar Negeri' : deleteDoc.program) 
    : 'Penelitian';

  const researchYear = deleteDoc?.tahun
    ? (() => {
        const str = String(deleteDoc.tahun).trim();
            ...
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted dark:text-on-dark-muted mt-1 leading-relaxed">
                  Tindakan ini tidak dapat dibatalkan. Data penelitian akan dihapus permanen dari sistem.
                </p>
              </div>
              ...
              <div className="w-full p-3.5 bg-surface-light-raised dark:bg-surface-dark-elevated rounded-xl border border-hairline-light dark:border-hairline-dark flex items-start gap-3 text-left">
                ...
                <div className="min-w-0 flex-1 space-y-1.5">
                  <p className="text-xs font-bold text-ink-heading dark:text-on-dark leading-snug line-clamp-2" title={deleteDoc.judul_penelitian}>
