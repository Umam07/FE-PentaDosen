                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-hairline-light-soft dark:border-hairline-dark-soft">
                      <th className="pb-2.5 font-semibold text-[10px] text-muted dark:text-on-dark-muted uppercase tracking-wider">Program Penelitian</th>
                      <th className="pb-2.5 font-semibold text-[10px] text-muted dark:text-on-dark-muted uppercase tracking-wider text-center">Rupiah Poin</th>
                  ...
                  <tbody className="divide-y divide-hairline-light-soft dark:divide-hairline-dark-soft text-body dark:text-on-dark-soft">
                    {researchMetrics.map((item) => (
                      <tr key={item.label} className="hover:bg-surface-light-raised/50 dark:hover:bg-surface-dark-elevated/40 transition-colors">
                        <td className="py-3 font-medium">
                          <div>
                            <span className="block font-semibold text-ink-heading dark:text-on-dark">{item.label}</span>
                            <span className="block text-[11px] text-muted dark:text-on-dark-muted mt-0.5">{item.desc}</span>
                          </div>
                        </td>
                        <td className="py-3 text-center text-muted dark:text-on-dark-muted tabular-nums">-</td>
                        <td className="py-3 text-right font-bold font-mono tabular-nums text-ink-heading dark:text-on-dark text-sm">
                          {item.pts} Pts
                        </td>
                      </tr>
                    ))}
            ...
            <div className="flex items-start gap-3.5 p-4 sm:p-5 bg-surface-light-raised dark:bg-surface-dark-elevated border border-hairline-light dark:border-hairline-dark rounded-2xl">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-body dark:text-on-dark-soft leading-relaxed">
                  ...
                  Dokumen yang masih berstatus <span className="font-semibold text-warning dark:text-warning-on-dark">Pending</span> atau <span className="font-semibold text-error dark:text-error-on-dark">Ditolak</span> tidak akan dihitung dalam total KPI.
                </p>
              </div>
            </div>
          </div>
