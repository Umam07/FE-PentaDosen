...
export default function Publication({ user }: { user: any }) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      points: Math.round(src.reduce((acc: number, d: any) => {
  ...
  return (
    <div className="max-w-none space-y-6 lg:space-y-10 pb-12">
      ...
      <DocumentDetailDrawer
        ...
        customMetadata={
          (activeDetailDoc?.quartile || activeDetailDoc?.author_role || activeDetailDoc?.citations !== undefined || activeDetailDoc?.is_corresponding !== undefined) && (
            <div className="col-span-2 pt-2 border-t border-gray-100 dark:border-zinc-800 space-y-2">
              <p className="text-[9px] font-black text-gray-400 dark:text-zinc-500 uppercase tracking-widest leading-none mb-1">
                {activeDetailDoc.source === 'scholar' ? 'Detail Google Scholar (Metrik SINTA)' : 'Detail Scopus (Metrik SINTA)'}
              </p>
              <div className="flex flex-wrap gap-2">
                {activeDetailDoc.source === 'scholar' && (
                  <span className="inline-flex items-center gap-1 text-[9px] font-black uppercase text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded-md border border-blue-100 dark:border-blue-900/30">
                    Sitasi: {activeDetailDoc.citations || 0}
                  </span>
                )}
                ...
                {activeDetailDoc.source !== 'scholar' && activeDetailDoc.is_corresponding !== undefined && (
