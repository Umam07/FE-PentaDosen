...
import { Link, ChevronRight, AlertCircle, Search, X, CheckCircle2, Unlink, Beaker, Calendar, Award } from 'lucide-react';
...
export default function HKILinkingModal({
  ...
  const handleLinkToResearchInternal = async (penelitianId: number | null) => {
    ...
    try {
      ...
      if (data.success) {
        onShowMessage(
          penelitianId ? 'Dokumen HKI berhasil dihubungkan ke penelitian!' : 'Tautan penelitian berhasil dilepas.',
          'success'
        );
        onClose();
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted dark:text-on-dark-muted truncate">
                    {docToLink ? `Target: "${docToLink.title}"` : 'Pilih data riset yang mendasari dokumen ini'}
                  </p>
                </div>
              </div>
                  ...
                  <div className="py-12 px-4 text-center rounded-2xl bg-surface-light-raised/40 dark:bg-surface-dark-elevated/30 border border-hairline-light-soft dark:border-hairline-dark-soft">
                    ...
                    <p className="text-xs text-muted dark:text-on-dark-muted mt-1 max-w-sm mx-auto">
                      {searchQuery
                        ...
                        : 'Daftarkan penelitian Anda pada menu Penelitian terlebih dahulu agar dapat dihubungkan ke HKI.'}
                    </p>
                  </div>
                )}
              </div>
