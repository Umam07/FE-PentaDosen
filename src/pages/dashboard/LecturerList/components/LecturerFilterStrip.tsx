      ...
      <div className="w-full sm:max-w-md relative" ref={dropdownRef}>
        <MotionConfig reducedMotion="user">
          ...
          <button
            ...
            aria-label="Filter dosen berdasarkan fakultas"
            aria-expanded={isOpen}
            aria-haspopup="listbox"
            className={`w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-xs font-semibold border transition-all cursor-pointer focus-visible:ring-2 focus-visible:ring-accent ${
              isOpen
                ? 'bg-surface-light dark:bg-surface-dark border-accent dark:border-accent-on-dark text-ink-heading dark:text-on-dark shadow-xs'
          ...
          >
            <div className="flex items-center gap-2.5 min-w-0">
              <Filter className="w-3.5 h-3.5 text-accent dark:text-accent-on-dark shrink-0" />
              <span className="text-[10px] font-bold text-muted dark:text-on-dark-muted uppercase tracking-wider shrink-0">
                Fakultas:
              </span>
              
              <div className="flex items-center gap-2 truncate">
                <span className={`w-2 h-2 rounded-full shrink-0 ${selectedDotColor}`} />
                <span className="truncate font-semibold text-ink-heading dark:text-on-dark">
                  {selectedFakultas}
                </span>
          ...
          <AnimatePresence>
            {isOpen && (
              <motion.div
                role="listbox"
                aria-label="Daftar pilihan fakultas"
                    ...
                    {fakultasOptions.map((fak) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className="relative flex w-full items-center justify-between px-3 py-2 text-xs font-medium rounded-xl cursor-pointer transition-colors duration-150"
                          whileTap={{ scale: 0.98 }}
